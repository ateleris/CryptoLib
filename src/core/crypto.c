...
int32_t Crypto_Process_Clear_TC_EP(uint8_t *frame, int len)
{
    ...
    uint8_t  vcid  = ((frame[2] & 0xFC) >> 2) & crypto_config.vcid_bitmask;
    ...
    status = Crypto_Get_Managed_Parameters_For_Gvcid(tfvn, scid, vcid, TYPE_TC, gvcid_managed_parameters_array, &mp);
    if (status != CRYPTO_LIB_SUCCESS)
    {
        mc_if->mc_log(status);
        return status;
    ...
    if (fecf_len && crypto_config.crypto_check_fecf == TC_CHECK_FECF_TRUE)
    {
